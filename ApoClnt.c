...
int ApoClnt_GetQuantNames(char ***quNames)
{
    if(EditQuants == NULL)
        return;
}
...
int ApoClnt_ContainsQuant(const char *name)
{
    if (SubsQuants == NULL)
	return 0;

    for(int i = 0;i<SubsQuants->n;i++)
    {
        if(strcmp(SubsQuants->Quants[i].Name, name) == 0)
        {
            return 1;
        }
    }
...
tApoClnt_Quantity* ApoClnt_GetQuant(const char *name)
{
    if (SubsQuants == NULL)
	return 0;
...
int ApoClnt_GetAllQuants(char ***quNames)
{
    tQuantList *qulist = EditQuants;
    if (qulist == NULL || qulist->Quants == NULL)
        return -1;
    *quNames = (char **)malloc(qulist->n * sizeof(char *));
    if (*quNames == NULL)
        return -1;
    for (int i = 0; i < qulist->n; i++)
    {
        (*quNames)[i] = strdup(qulist->Quants[i].Name);
        if ((*quNames)[i] == NULL)
        {
            printf("strdup failed Name : %s\n ", qulist->Quants[i].Name);
